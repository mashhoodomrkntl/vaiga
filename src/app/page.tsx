...
export default function HomePage() {
  ...
  const currentHero = heroContent[country];

  // Filter services if needed, but for now they seem global with country-specific details inside
  // Testimonials filtering
  const filteredTestimonials = testimonials.filter(t =>
    country === "Global" || t.location.includes(country)
  );

  // Stats can be adjusted or kept global
  const filteredStats = country === "Global" ? stats : stats.map(s => {
  ...
  ].filter(o => country === "Global" || o.country === country);
    ...
    <>
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden bg-primary/15">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          {/* Subtle Geometric Pattern - Very faint for premium feel */}
          ...
          >
            ...
            >
              ...
              <span className="text-primary-800 text-[10px] font-bold uppercase tracking-[0.25em]">
                {country === "Global" ? "Global Consultancy Excellence" : `${country} Operations`}
              </span>
            </motion.div>
      ...
      <section id="services" className="py-20 md:py-28 bg-surface-secondary">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-12">
          <motion.div {...fadeInUp} className="text-center mb-14">
            ...
            <p className="text-text-secondary max-w-2xl mx-auto text-sm md:text-base">
              {country === "Global"
                ...
                : `Comprehensive financial and consultancy solutions tailored to empower businesses in ${country}.`}
            </p>
          </motion.div>
          ...
          <motion.div {...fadeInUp} className="text-center mt-12">
            <Link
              href={country === "Global" ? "/services" : `/services/${country.toLowerCase()}`}
              className="inline-flex items-center gap-2 text-primary font-semibold text-sm hover:gap-3 transition-all"
            >
              Explore All {country === "Global" ? "" : country} Services
              <ArrowRight size={16} />
            </Link>
          </motion.div>
        </div>
      ...
      <section className="py-20 md:py-28 bg-surface-secondary">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-12">
          <motion.div {...fadeInUp} className="text-center mb-14">
            <span className="inline-block text-primary font-semibold text-sm uppercase tracking-wider mb-3">
              {country === "Global" ? "Global Reach" : `${country} Focus`}
            </span>
            <h2 className="font-[var(--font-heading)] text-3xl md:text-4xl lg:text-5xl font-bold text-text-primary mb-4">
              {country === "Global" ? "Our Global Presence" : `Our Presence in ${country}`}
            </h2>
            <p className="text-text-secondary max-w-2xl mx-auto text-sm md:text-base">
              {country === "Global"
                ? "Strategically located across three countries to serve your business wherever you operate."
                : `Strategically located in ${country} to provide specialized local expertise with global standards.`}
            </p>
          </motion.div>
          ...
          <motion.div {...fadeInUp} className="text-center mb-14">
            ...
            <p className="text-text-secondary max-w-2xl mx-auto text-sm md:text-base">
              {country === "Global"
                ...
                : `Hear from businesses in ${country} who trust Vaiga for their financial and advisory needs.`}
            </p>
          </motion.div>
            ...
            <div className="grid gap-6">
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                {[
                  ...
                  {
                    icon: Globe,
                    title: country === "Global" ? "Global Reach" : `${country} Reach`,
                    desc: country === "Global" ? "India, UAE, Qatar presence." : `Strategic ${country} presence.`
                  }
                ].map((item, index) => (
                  <motion.div
                    key={item.title}
